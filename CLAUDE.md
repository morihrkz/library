...
- 日本語の地の文・見出し・図注・注記で、文の切れ目や副題を区切るために使うダッシュは、 em-dash( `—` U+2014 )ではなく前後に半角スペースを挟んだハイフン( ` - ` )で書く。 `——` のような em-dash の連続も同じく ` - ` にまとめる。
  ...
  - 理由: 2026-08-25 、記事間でダッシュの表記( em-dash と半角ハイフン )が割れていたため、ディレクトリの標準を ` - ` に定めた。 em-dash は環境によって字幅と前後アキの見え方が大きく変わり、和文中では前後の空きが詰まって読みにくくなる。ハイフンに半角スペースを添える形なら、和欧間アキのルールともそのまま整合する。
- 対象外(そのまま残す):
...
- 唯一の例外は記事題名(`header.hero h1`)。ここだけは本文の約 3 倍のサイズに拡大し、セリフ体にする。上の 2 つの `<style>` ブロックより後に、次を追記する(数値は既存記事間で多少ぶれるが、下記が標準)。
  ...
  - 理由: 2026-08-29 、ディレクトリ内 31 記事にこの上書きが既に導入済み(コミット `88bb00e` 「記事題名(h1)を本文の約 3 倍・セリフ体に拡大し、ヒーロー構造を header.hero に統一」)であることが判明したが、本節にその例外が明文化されていなかったため追記した。新規記事もヒーローの `h1` にはこの上書きを最初から含める。
...
- 読点(、)の位置に `<br>` を挿入して段落内で強制改行するルールは廃止した(2026-08-20)。
  - 理由: ある記事で読点直後に `<br>` を機械的に挿入した箇所が 85 件見つかり、レイアウトを読者のウィンドウ幅に委ねず本文中に手動で改行を埋め込む手法自体が不適切と判断したため。既存の適用箇所は削除済み。
...
- **アニメーション・動的描画は持ち込まない。** `<canvas>` 、`requestAnimationFrame` 、手続き的な模様生成、ヒーロー専用の `<script>` はいずれも使わない。
  - 理由: 2026-08-25 、ある記事が全画面の canvas ヒーロー(星の明滅と CMB ゆらぎ場の生成、専用 JS 約 100 行)を持っており、他記事と作りが揃っていなかったため標準形へ書き換えた。静的 SVG なら図の内容がマークアップとして残り、読み上げ・印刷・差分レビューのいずれでも同じものが見える。装飾のために JS を動かす必要もない。
...
- 複数の章(`第 N 章` 等の見出し単位)を持つ長文記事は、本文をアコーディオン(開閉式)にする。これはディレクトリ内の大半の記事がすでに採用している標準仕様であり、新規作成時は最初から適用する。単一の短い記事(章立てのないエッセイ等)には適用しなくてよい。
  - 理由: 2026-08-25 、ある新規記事にこの機構が抜けていることをユーザーに指摘された。以後は指摘を待たずに標準仕様として適用する。
- 実装(次の構造・クラス名を用いる):
...
- 全記事で `prefers-color-scheme: dark` に対応する。 2026-08-25 、ディレクトリ内 25 記事のうち対応済みが 5 記事のみで、残り 20 記事が白地固定だった。読者のブラウザ・ OS 設定に応じて夜間は暗い配色になることをディレクトリの標準方針とし、この日に未対応 20 記事へ一括適用した。
- 実装パターン(次の方式に従う):
