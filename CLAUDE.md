...
- 出典一覧は `<ol class="refs">`(クラス名は記事ごとに任意でよいが、意味の分かる名にする)にし、`list-style:none;padding-left:0` を指定して自動連番を消す。各 `<li>` の先頭に手動で `<a class="fn-num" href="#{本文側の forward link の id}">{N}.</a>` を追加する。`.fn-num` には色・太字を指定しない(見た目の節を参照)。
